/*------------------------------------------------------------------------------
    ...
    Syntax      : RUN WinLdapAuth.p PERSISTENT SET m_hLDAPAuth
                                            ( INPUT p_cBindServer,
                                              INPUT p_cBindPort,
...
DEFINE VARIABLE m_cBindPort             AS CHARACTER INITIAL "389" NO-UNDO.
DEFINE VARIABLE m_cUserSearchRootDN     AS CHARACTER INITIAL "" NO-UNDO.
DEFINE VARIABLE m_cUserSearchFilter     AS CHARACTER INITIAL "(&(objectclass=user)(sAMAccountName=%s))" NO-UNDO.
...
DEFINE VARIABLE m_iOSPointerSize        AS INTEGER INITIAL 4 NO-UNDO.
...
PROCEDURE authenticate :
    ...
    IF (m_lTrace) THEN 
        MESSAGE "trace: authenticating user " + p_cUser view-as alert-box.
    ...
    IF ( ? <> p_cUser AND
        ...
        **       Most auditors will NOT accept a product that does this */
        IF (m_lTrace) THEN 
            MESSAGE "trace: Authenticating user DN : " + cUserDN view-as alert-box.
    
        RUN ldap_bind_sA ( INPUT  m_mLDAPContext,
                           INPUT  cUserDN,
                           INPUT  p_cPassword,
                           INPUT  m_iAuthMethod,
        ...
        IF (m_lDebug) THEN 
            MESSAGE "debug: LDAP simple bind returned " + 
                    " : " + STRING(iStatus) view-as alert-box.

        IF ( 0 <> iStatus ) THEN DO:
            RUN ldap_err2stringA( INPUT  iStatus, OUTPUT m_mLastLDAPError ) /* NO-ERROR */.
    ...
    IF ( m_lBindComplete ) THEN DO:
        IF (m_lTrace) THEN 
            MESSAGE "trace: Unbinding from LDAP server" view-as alert-box.
...
PROCEDURE resolveUserDN PRIVATE :
    ...
    DEFINE VARIABLE lEntry                  AS INTEGER NO-UNDO.
    DEFINE VARIABLE lRes                    AS INTEGER NO-UNDO.
    ...
    IF (m_lTrace) THEN 
        MESSAGE "trace: resolving user account DN for " + p_cUserId view-as alert-box.
    ...
    IF ( ? <> p_cUserId AND
         ...
         IF ( "" <> m_cSearchBindDN ) THEN DO:
             IF (m_lTrace) THEN 
                 MESSAGE "trace: Binding search user: " + m_cSearchBindDN view-as alert-box.
             ...
             IF (m_lDebug) THEN 
                 MESSAGE "debug: LDAP search bind returned " + 
                         " : " + STRING(iBindStatus) view-as alert-box.
     
             IF ( 0 <> iBindStatus ) THEN DO:
                 RUN ldap_err2stringA( INPUT  iBindStatus, 
        ...
        RUN ldap_search_sA ( INPUT  m_mLDAPContext,
         ...
         * the search */
        SET-POINTER-VALUE(mLDAPMsg) = lRes.
        ...
        ** exists in more than one sub-tree of the search, and an error*/
        ...
        IF (m_lTrace) THEN 
            MESSAGE "trace: number of LDAP matches for user-id is " + 
                    STRING(iReturns) view-as alert-box.
        ...
        /* Get the memory pointer to the LDAPMessage structure's first entry */
        SET-POINTER-VALUE(mEntry) = lEntry.
        ...
        IF (m_lTrace) THEN 
            MESSAGE "trace: getting LDAP DN user-id " + 
                    p_cUserId view-as alert-box.
        RUN ldap_get_dnA ( INPUT  m_mLDAPContext,
                           INPUT  mEntry,
                           OUTPUT mDN ) NO-ERROR.
        ...
        IF (m_lDebug) THEN 
            MESSAGE "debug: resolved LDAP DN for user-id " + p_cUserID +
                    " is " + GET-STRING(mDN,1) view-as alert-box.
        ...
        IF (m_lDebug) THEN 
            MESSAGE "debug: Releasing LDAP DN memory" view-as alert-box.
        RUN ldap_memfreeA ( INPUT  mDN ) /* NO-ERROR */.        
    END. ELSE DO:
    ...
    IF ( lFreeEntry ) THEN DO:
        IF (m_lDebug) THEN 
            MESSAGE "debug: Releasing LDAP search result" view-as alert-box.
        RUN ldap_msgfree( INPUT  mLDAPMsg,
                          OUTPUT iStatus ) /* NO-ERROR */.
        IF (m_lDebug) THEN 
            MESSAGE "after debug: Releasing LDAP search result - " +  string(iStatus) view-as alert-box.
    END.
    
    IF (m_lDebug) THEN 
        MESSAGE "before cleaning mDN" view-as alert-box.
    
    /* Clean up all OS memory pointer allocations */
    if get-size(mDN) > 0 then
        SET-SIZE(mDN) = 0.
    
    /*
    IF (m_lDebug) THEN 
        MESSAGE "before cleaning mEntry" view-as alert-box.
        
    if get-size(mEntry) > 0 then 
        SET-SIZE(mEntry) = 0.
    */
    
    /*
    IF (m_lDebug) THEN 
        MESSAGE "before cleaning mLDAPMsg" view-as alert-box.
    
    if get-size(mLDAPMsg) > 0 then
        SET-SIZE(mLDAPMsg) = 0.
    */
    
    IF (m_lDebug) THEN 
        MESSAGE "after cleaning" view-as alert-box.

    /* Return the results of the user account DN search */
    
    IF (m_lDebug) THEN 
        MESSAGE "before return" view-as alert-box.
...
PROCEDURE resolveMemberships :
    ...
    IF (m_lTrace) THEN 
        MESSAGE "trace: searching group memberships for user-id " + p_cUserDN view-as alert-box.
    ...
    IF ( ? <> p_cUserDN AND
         ...
         ** returned by the shared library. */
         SET-SIZE(mEntry) = m_iOSPointerSize.
         PUT-BYTES(mEntry, 1) = m_mOSNullPointer.
         SET-SIZE(mEntryMsg) = m_iOSPointerSize.
         PUT-BYTES(mEntryMsg, 1) = m_mOSNullPointer.
         SET-SIZE(mLDAPMsg) = m_iOSPointerSize.
         PUT-BYTES(mLDAPMsg, 1) = m_mOSNullPointer.
        ...
        IF (m_lTrace) THEN 
            MESSAGE "trace: searching for groups for " + p_cUserDN + " using " +
                    ...
                    cGroupFilter view-as alert-box.
        ...
        ** shared library on procedure exit. */
        ...
        IF ( 0 = iReturns ) THEN DO:
            IF (m_lDebug) THEN 
                MESSAGE "debug: no group memberships found for " + p_cUserDN view-as alert-box.
            LEAVE.
        END.
        ...
        ** search */
        DO iEntryIdx = 1 TO iReturns:
            ...
            /* Get the pointer to the ldap message structure allocated by the
            ** shared library */
            SET-POINTER-VALUE(mEntryMsg) = lEntry.

            RUN ldap_get_dnA ( INPUT  m_mLDAPContext,
                               INPUT  mEntryMsg,
                               OUTPUT mDN ) NO-ERROR.
            IF (m_lDebug) THEN
                MESSAGE "debug: returned group DN: " + GET-STRING(mDN, 1) view-as alert-box.
            ...
            ** For a cn we should only get 1 value */
            RUN ldap_get_valuesA ( INPUT  m_mLDAPContext,
                                   INPUT  mEntryMsg,
                                   INPUT  mAttrName,
            ...
            RUN LdapGetLastError(OUTPUT iStatus).
            IF (m_lDebug) THEN 
                MESSAGE "debug: return status for attribute value: " + STRING(iStatus) view-as alert-box.
            RUN LdapGetLastError(OUTPUT iStatus).
            IF ( 0 <> iStatus ) THEN DO:
                RUN ldap_err2stringA( INPUT  iStatus, OUTPUT m_mLastLDAPError ) /* NO-ERROR */.
            ...
            ** check */
            ...
            RUN ldap_count_valuesA ( INPUT  mAttrValues,
                                    OUTPUT iValueCount ) /* NO-ERROR */.
            IF (m_lDebug) THEN 
                MESSAGE "debug: found " + STRING(iValueCount) + 
                        " attribute values for cn" view-as alert-box.

            /* Add the name of the LDAP group (the cn attribute) to the list of
             * group names the user is a member of */
            SET-POINTER-VALUE(mAttrValue) = GET-LONG(mAttrValues, 1).
            IF (m_lDebug) THEN 
                MESSAGE "debug: recoverd group cn: " + GET-STRING(mAttrValue,1) view-as alert-box.
            cGroupList = cGroupList + cDelim + GET-STRING(mAttrValue,1).
            cDelim = ",".

            /* Free up the memory allocated for the LDAP attribure values */
            IF (m_lDebug) THEN 
                MESSAGE "debug: Relasing LDAP values " view-as alert-box.
            RUN ldap_value_freeA ( INPUT mAttrValues, OUTPUT iStatus) /* NO-ERROR */.

            /* Get the next LDAP search entry */
            IF (m_lTrace) THEN 
                MESSAGE "trace: Getting next LDAP search entry ..." view-as alert-box.
...
PROCEDURE resolveADStatus :
    ...
    DEFINE VARIABLE lDone                   AS LOGICAL INITIAL NO NO-UNDO.
    DEFINE VARIABLE lFreeEntry              AS LOGICAL INITIAL NO NO-UNDO.
    DEFINE VARIABLE lRes                    AS INTEGER INITIAL 0 NO-UNDO.
    DEFINE VARIABLE lEntry                  AS INTEGER INITIAL 0 NO-UNDO.
    DEFINE VARIABLE mDN                     AS MEMPTR NO-UNDO.
    DEFINE VARIABLE mLDAPMsg                AS MEMPTR NO-UNDO.
    DEFINE VARIABLE mEntry                  AS MEMPTR NO-UNDO.
    DEFINE VARIABLE iReturns                AS INTEGER INITIAL 0 NO-UNDO.
    ...
    IF (m_lTrace) THEN 
        MESSAGE "trace: resolving Active Directory status for user-id " + p_cUserDN view-as alert-box.
    ...
    IF ( ? <> p_cUserDN AND
        ...
        ** than one, the simple user-id is ambiguous, and an error*/
        ...
        IF ( 0 = iReturns ) THEN DO:
            IF (m_lDebug) THEN 
                MESSAGE "debug: no AD user account found to obtains status for " + p_cUserDN view-as alert-box.
            LEAVE.
        END.
        ...
        ** one. */
        ...
        RUN LdapGetLastError(OUTPUT iStatus).
        IF (m_lDebug) THEN 
            MESSAGE "debug: return status for attribute userAccountControl : " + STRING(iStatus) view-as alert-box.
        IF ( 0 <> iStatus ) THEN DO:
            RUN ldap_err2stringA( INPUT  iStatus, OUTPUT m_mLastLDAPError ) /* NO-ERROR */.
        ...
        IF (m_lDebug) THEN 
            MESSAGE "debug: return account status userAccountControl : " + 
                    STRING(iAcctStatus) view-as alert-box.
        ...
        ** shared library */
        IF (m_lDebug) THEN 
            MESSAGE "debug: Releasing LDAP attribute value " view-as alert-box.
        RUN ldap_value_freeA ( INPUT mAttrValues, OUTPUT iStatus) /* NO-ERROR */.
    ...
    IF ( lFreeEntry ) THEN DO:
        IF (m_lDebug) THEN 
            MESSAGE "debug: Relasing LDAP search result" view-as alert-box.
        RUN ldap_msgfree( INPUT  mLDAPMsg,
                          OUTPUT iStatus ) /* NO-ERROR */.
    END.
...
PROCEDURE loadAccountAttributes PRIVATE :
    ...
    IF (m_lTrace) THEN 
        MESSAGE "trace: loading attributes for user-id " + p_cUserDN view-as alert-box.
    ...
    ** Obtain how many attributes to get ( 0 equals all attributes ) */
    ...
    DO WHILE NOT (lDone):
        ...
        IF ( "*" = cAttrList ) THEN DO:
            IF (m_lTrace) THEN 
                MESSAGE "trace: searching for wildcard attributes using " +
                        p_cUserDN + " and filter " + cUserFilter view-as alert-box.
            RUN ldap_search_sA ( INPUT  m_mLDAPContext,
                                 INPUT  p_cUserDN,
                                 INPUT  {&LDAP_SCOPE_BASE},
                                 INPUT  cUserFilter,
        ...
        ** search.  If we get more than one, the user-id is ambiguous, and an error*/
        ...
        IF (m_lTrace) THEN 
            MESSAGE "trace: getting first LDAP attribute name... " view-as alert-box.
        ...
        ** from the shared library. */
        RUN ldap_first_attributeA ( INPUT  m_mLDAPContext,
                                    INPUT  mEntry,
        ...
        /* Get the pointer to the LDAPMessage structure returned by the search */
        SET-POINTER-VALUE(mAttrCtx) = GET-LONG(mAttrCtxHolder,1).
        ...
        IF ( ? = cAttrName OR
             "" = cAttrName ) THEN DO:
            IF (m_lDebug) THEN 
                MESSAGE "debug: no LDAP attributes found" view-as alert-box.
            lAttrDone = YES.
        END.
        ...
        DO WHILE NOT (lAttrDone):
            ...
            IF (m_lTrace) THEN 
                MESSAGE "trace: getting value for attribute: " + cAttrName view-as alert-box.
            ...
            ** attribute type ). */
            ...
            IF (m_lDebug) THEN 
                MESSAGE "debug: found " + STRING(iValueCount) + 
                        " attribute values for " + cAttrName view-as alert-box.

            /* Now loop and extract the attribute values from an array of 
            ** character string pointers */
            iPtrOffset = 1.
            DO iAttrIndex = 1 TO iValueCount:
                SET-POINTER-VALUE(mAttrValue) = GET-LONG(mAttrValues, iPtrOffset).
                IF (m_lDebug) THEN 
                    MESSAGE "debug: attribute value for " + cAttrName + "[" +
                            STRING(iAttrIndex) + "] is: " + GET-STRING(mAttrValue,1) view-as alert-box.

                cRetAttrs = cRetAttrs + cAttrDelim + cAttrName + "=" + 
                            GET-STRING(mAttrValue,1).
                cAttrDelim = "&".
                iPtrOffset = iPtrOffset + 4 /* GET-LONG */.
            END.

            /* Free up the memory allocated for the LDAP attribure values */
            IF (m_lDebug) THEN 
                MESSAGE "debug: Relasing LDAP attribute values " view-as alert-box.
            RUN ldap_value_freeA ( INPUT mAttrValues, OUTPUT iStatus) /* NO-ERROR */.

            /* Give back the LDAP allocated memory when we are done with the
             * attribute name */
            cAttrName = ?.
            RUN ldap_memfreeA (mAttrName) /* NO-ERROR */.

            IF (m_lTrace) THEN 
                MESSAGE "trace: getting next LDAP attribute name ... " view-as alert-box.

            /* Get the next (multi-value) attribute to process */
            RUN ldap_next_attributeA ( INPUT  m_mLDAPContext,
                                       INPUT  mLDAPMsg,
                                       INPUT  mAttrCtx,
                                       OUTPUT mAttrName ) /* NO-ERROR */.
            ...
            cAttrName = GET-STRING(mAttrName, 1) NO-ERROR.
                IF ( ? <> cAttrName AND
                     "" <> cAttrName ) THEN DO:
                    IF (m_lTrace) THEN 
                        MESSAGE "trace: next attr name is: " + cAttrName view-as alert-box.
                END. ELSE DO:
            ...
            END. ELSE DO:
                IF (m_lTrace) THEN 
                    MESSAGE "trace: end of LDAP attribute names" view-as alert-box.
                LEAVE.
            END.
    ...
    ** library */
    IF ( lFreeEntry ) THEN DO:
        IF (m_lTrace) THEN 
            MESSAGE "trace: Cleaning up LDAP search result" view-as alert-box.
        RUN ldap_msgfree( INPUT  mLDAPMsg,
                          OUTPUT iStatus ) /* NO-ERROR */.
    END.
...
PROCEDURE defaultConstructor PRIVATE :
    ...
    IF (m_lTrace) THEN 
        MESSAGE "trace: Enter LDAP default constructor" view-as alert-box.
   ...
   ** sub-trees are access protected from the anonymous bind user */
    ...
    ** it needs to.  */
    DO WHILE (NOT lDone ) :
        ...
        IF (m_lTrace) THEN 
            MESSAGE "trace: Initializing LDAP ..." view-as alert-box.
    
        SET-SIZE(m_mLDAPContext) = m_iOSPointerSize.
        PUT-BYTES(m_mLDAPContext, 1) = m_mOSNullPointer.

        /* Initialize the LDAP connection information */
        RUN ldap_sslinitA ( INPUT  m_cBindServer,
        ...
        IF (m_lTrace) THEN 
            MESSAGE "trace: Setting LDAP version 3..." view-as alert-box.
        ...
        /* The ldap server spec is a LDAP server DNS name, (not an AD domain 
        ** that is supported by the Windows shared library)*/
        IF (m_lTrace) THEN 
            MESSAGE "trace: Setting LDAP DNS host lookup..." view-as alert-box.
        PUT-LONG(mOptionValue,1) = {&LDAP_OPT_ON}.
        RUN ldap_set_option ( INPUT  m_mLDAPContext,
                              INPUT  {&LDAP_OPT_AREC_EXCLUSIVE},
                              INPUT  mOptionValue,
                              OUTPUT iStatus) NO-ERROR.
        IF ( 0 <> iStatus ) THEN DO:
            RUN ldap_err2stringA( INPUT  iStatus, OUTPUT m_mLastLDAPError ) /* NO-ERROR */.
            ...
            DO i = 1 TO iPropCount :
                ...
                IF ( 8 < LENGTH(cPropName) AND
                    ...
                    IF ( ERROR-STATUS:ERROR ) THEN DO:
                        IF (m_lDebug) THEN DO:
                            MESSAGE "LDAP option name " + cPropName + " was invalid" view-as alert-box.
                            NEXT.
                        END.
                    END.
                    ...
                    IF ( ? <> cPropValue AND
                        ...
                        IF ( 0 <> iStatus ) THEN DO:
                            RUN ldap_err2stringA( INPUT  iStatus, 
                                                  OUTPUT m_mLastLDAPError ) /* NO-ERROR */.
                    ...
                    END. ELSE DO:
                        IF (m_lDebug) THEN DO:
                            MESSAGE "LDAP option " + cPropName + " had no value" view-as alert-box.
                        END.   
                    END.
                END.
            END.
...
PROCEDURE ldap_sslinitA EXTERNAL "wldap32.dll" PERSISTENT CDECL.
    DEFINE INPUT  PARAMETER p_cServerPort    AS CHARACTER NO-UNDO.
    DEFINE INPUT  PARAMETER p_mPort          AS LONG NO-UNDO.
    DEFINE INPUT  PARAMETER p_iEnableSSL     AS LONG NO-UNDO.
    DEFINE RETURN PARAMETER p_mLDAPContext   AS MEMPTR NO-UNDO.
END PROCEDURE.

PROCEDURE ldap_set_option EXTERNAL "wldap32.dll" PERSISTENT CDECL.
...
PROCEDURE ldap_get_option EXTERNAL "wldap32.dll" PERSISTENT CDECL.
    DEFINE INPUT  PARAMETER p_mLDAPContext      AS MEMPTR NO-UNDO.
    DEFINE INPUT  PARAMETER p_iLDAPOption       AS LONG NO-UNDO.
...
PROCEDURE ldap_bind_sA EXTERNAL "wldap32.dll" PERSISTENT CDECL.
    DEFINE INPUT  PARAMETER p_mLDAPContext      AS MEMPTR NO-UNDO.
...
PROCEDURE ldap_search_sA EXTERNAL "wldap32.dll" PERSISTENT CDECL.
    DEFINE INPUT  PARAMETER p_mLDAPContext      AS MEMPTR NO-UNDO.
    ...
    /* DEFINE INPUT-OUTPUT PARAMETER p_mLDAPMessage AS MEMPTR NO-UNDO. */
    DEFINE OUTPUT PARAMETER p_mLDAPMessage      AS HANDLE TO LONG NO-UNDO.
    DEFINE RETURN PARAMETER p_iStatus           AS LONG NO-UNDO.
END PROCEDURE.

PROCEDURE ldap_unbind_s EXTERNAL "wldap32.dll" PERSISTENT CDECL.
    DEFINE INPUT  PARAMETER p_mLDAPContext      AS MEMPTR NO-UNDO.
    DEFINE RETURN PARAMETER p_iStatus           AS LONG NO-UNDO.
END PROCEDURE.

PROCEDURE ldap_count_entries EXTERNAL "wldap32.dll" PERSISTENT CDECL.
    DEFINE INPUT  PARAMETER p_mLDAPContext      AS MEMPTR NO-UNDO.
    DEFINE INPUT  PARAMETER p_mLDAPMessage      AS MEMPTR NO-UNDO.
    DEFINE RETURN PARAMETER p_iStatus           AS LONG NO-UNDO.
END PROCEDURE.

PROCEDURE ldap_msgfree EXTERNAL "wldap32.dll" PERSISTENT CDECL.
    DEFINE INPUT  PARAMETER p_mLDAPMessage      AS MEMPTR NO-UNDO.
    DEFINE RETURN PARAMETER p_iStatus           AS LONG NO-UNDO.
END PROCEDURE.

PROCEDURE ldap_first_entry EXTERNAL "wldap32.dll" PERSISTENT CDECL.
    DEFINE INPUT  PARAMETER p_mLDAPContext      AS MEMPTR NO-UNDO.
    DEFINE INPUT  PARAMETER p_mLDAPMessage      AS MEMPTR NO-UNDO.
    DEFINE RETURN PARAMETER p_mLDAPEntry        AS HANDLE TO LONG NO-UNDO.
END PROCEDURE.

PROCEDURE ldap_next_entry EXTERNAL "wldap32.dll" PERSISTENT CDECL.
    DEFINE INPUT  PARAMETER p_mLDAPContext      AS MEMPTR NO-UNDO.
    DEFINE INPUT  PARAMETER p_mLDAPMessage      AS MEMPTR NO-UNDO.
    DEFINE RETURN PARAMETER p_mLDAPEntry        AS HANDLE TO LONG NO-UNDO.
END PROCEDURE.

PROCEDURE ldap_first_attributeA EXTERNAL "wldap32.dll" PERSISTENT CDECL.
    DEFINE INPUT  PARAMETER p_mLDAPContext      AS MEMPTR NO-UNDO.
    DEFINE INPUT  PARAMETER p_mLDAPMessage      AS MEMPTR NO-UNDO.
...
PROCEDURE ldap_next_attributeA EXTERNAL "wldap32.dll" PERSISTENT CDECL.
    DEFINE INPUT        PARAMETER p_mLDAPContext      AS MEMPTR NO-UNDO.
    DEFINE INPUT        PARAMETER p_mLDAPMessage      AS MEMPTR NO-UNDO.
...
PROCEDURE ldap_get_dnA EXTERNAL "wldap32.dll" PERSISTENT.
    DEFINE INPUT  PARAMETER p_mLDAPContext      AS MEMPTR NO-UNDO.
    DEFINE INPUT  PARAMETER p_mLDAPMessage      AS MEMPTR NO-UNDO.
    DEFINE RETURN PARAMETER p_mAttrName         AS MEMPTR NO-UNDO.
END PROCEDURE.

PROCEDURE ldap_memfreeA EXTERNAL "wldap32.dll" PERSISTENT CDECL.
    DEFINE INPUT PARAMETER p_mAttrMemory        AS MEMPTR NO-UNDO.
END PROCEDURE.

PROCEDURE ldap_get_valuesA EXTERNAL "wldap32.dll" PERSISTENT CDECL.
    DEFINE INPUT  PARAMETER p_mLDAPContext      AS MEMPTR NO-UNDO.
    DEFINE INPUT  PARAMETER p_mLDAPMessage      AS MEMPTR NO-UNDO.
...
PROCEDURE ldap_value_freeA EXTERNAL "wldap32.dll" PERSISTENT CDECL.
    DEFINE INPUT  PARAMETER p_mValueMemory      AS MEMPTR NO-UNDO.
    DEFINE RETURN PARAMETER p_iStatus           AS LONG NO-UNDO.
END PROCEDURE.

PROCEDURE ldap_count_valuesA EXTERNAL "wldap32.dll" PERSISTENT CDECL.
    DEFINE INPUT  PARAMETER p_mValueMemory      AS MEMPTR NO-UNDO.
    DEFINE RETURN PARAMETER p_iCount            AS LONG NO-UNDO.
END PROCEDURE.

PROCEDURE ldap_err2stringA EXTERNAL "wldap32.dll" PERSISTENT CDECL.
    DEFINE INPUT  PARAMETER p_iErrorCode        AS LONG NO-UNDO.
    DEFINE RETURN PARAMETER p_mErrorString      AS MEMPTR NO-UNDO.
END PROCEDURE.

PROCEDURE LdapGetLastError EXTERNAL "wldap32.dll" PERSISTENT CDECL.
    DEFINE RETURN PARAMETER p_lLastError        AS LONG NO-UNDO.
END PROCEDURE.

PROCEDURE ber_free EXTERNAL "wldap32.dll" PERSISTENT CDECL.
