/*------------------------------------------------------------------------------
    ...
    Syntax      : RUN UnixLdapAuth.p PERSISTENT SET m_hLDAPAuth
                                            ( INPUT p_cBindServer,
                                              INPUT p_cBindPort,
...
DEFINE VARIABLE m_cUserSearchFilter     AS CHARACTER INITIAL "(&(objectclass=person)(|(cn=%s)(uid=%s)))" NO-UNDO.
...
DEFINE VARIABLE m_serverctrls           AS MEMPTR             NO-UNDO.
DEFINE VARIABLE m_clientctrls           AS MEMPTR             NO-UNDO.
DEFINE VARIABLE m_timeout               AS MEMPTR             NO-UNDO.
...
PROCEDURE resolveUserDN PRIVATE :
    ...
    IF ( ? <> p_cUserId AND
        ...
        /***
        run ldap_search_ext_s ( INPUT  m_mLDAPContext,
                                INPUT  m_cUserSearchRootDN,
                                INPUT  {&LDAP_SCOPE_SUBTREE},
                                INPUT  cUserFilter,
                                INPUT  m_mOSNullPointer,
...
PROCEDURE ldap_search_ext_s EXTERNAL "libldap.so" PERSISTENT CDECL.
    DEFINE INPUT  PARAMETER p_mLDAPContext      AS MEMPTR NO-UNDO.
    DEFINE INPUT  PARAMETER p_cSearchRoot       AS CHARACTER NO-UNDO.
    DEFINE INPUT  PARAMETER p_iScope            AS LONG NO-UNDO.
    DEFINE INPUT  PARAMETER p_cSearchFilter     AS CHARACTER NO-UNDO.
    DEFINE INPUT  PARAMETER p_mAttrArray        AS MEMPTR NO-UNDO.
    DEFINE INPUT  PARAMETER p_iAttrsOnly        AS LONG NO-UNDO.
    DEFINE INPUT  PARAMETER p_serverctrls       AS LONG NO-UNDO.
    DEFINE INPUT  PARAMETER p_clientctrls       AS LONG NO-UNDO.
    DEFINE INPUT  PARAMETER P_timeout           AS CHARACTER NO-UNDO.
    /* DEFINE INPUT-OUTPUT PARAMETER p_mLDAPMessage AS MEMPTR NO-UNDO. */
    DEFINE OUTPUT PARAMETER p_mLDAPMessage AS HANDLE TO LONG NO-UNDO.
    DEFINE RETURN PARAMETER p_iStatus           AS LONG NO-UNDO.
END PROCEDURE.
